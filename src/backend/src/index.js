/* ────────────────────────────────────────────────────────────────
   ...
   ─ builds a read-only Git clone
   ─ exposes lightweight JSON endpoints for the React UI
   ─ delegates all Git-write operations to the helper script / CI
   ─────────────────────────────────────────────────────────────── */
...
/* ─── clone repo on boot (non-blocking) ──────────────────────── */
ensureRepo()
...
/* ============================================================= */
/* 3.  ArtifactHub search (≥ 4 chars, simple 1 h cache)           */
/* ============================================================= */
...
/* ============================================================= */
/* 4.  Details – default / override values + chart meta           */
/* ============================================================= */
app.get("/api/app/values", async (req, res) => {
  ...
  const repoRoot   = await ensureRepo();           // already cached
  const fileDir    = path.dirname(file);           // same dir as YAML
  const overrideYF = path.join(fileDir, VALUES_SUBDIR, `${name}.yaml`);
  ...
  const defaultYF = path.join(chartDir, "values.yaml");
  const chartYaml = path.join(chartDir, "Chart.yaml");

  let overrideY = "", defaultY = "", meta = {};
  try { overrideY = await fs.readFile(overrideYF, "utf8"); } catch {}
  try { defaultY  = await fs.readFile(defaultYF, "utf8");  } catch {}
  try {
    const m = yaml.load(await fs.readFile(chartYaml, "utf8")) || {};
...
app.post("/api/apps", async (req, res) => {
  ...
  /* fetch chart defaults via `helm show values …` --------------- */
